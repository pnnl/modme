    ...
    accept: function() {
        ...
        chart.response.forEach(function(d){d({domID:"accept", ascii: chart.data.controller.accept,
            channel:chart.data.channels[chart.select].name, frequency:chart.data.channels[chart.select].frequency,
                correct:chart.data.target.name == chart.data.channels[chart.select].name && chart.data.target.frequency==chart.data.channels[chart.select].frequency});});
...
d3.chart("Monitoring", {
    ...
    getSliderPercentage: function(sliderNum){
        var chart = this;
        ...
        var percentage = (currentPosition-(sliderCenter-0.25))/(chart.ticks-1)
        return percentage;
    },
